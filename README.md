...
| 模型 | 状态 |
| --- | --- |
| `glm-5.3` | ✅ 可用（工具调用、流式都已验证） |
| `deepseek-v4-flash` | ✅ 可用（同上） |
| `gpt-5.6-sol`、`gpt-6-astra`、`claude-opus-4-8`、`claude-opus-5` | ⛔ 返回 `402 Budget pool quota has been exhausted` |

402 是账号/额度层面的限制，不是配置问题；额度恢复后这几个会直接可用。想先把它们从选择器里藏掉，
就把 `cordis.patch.yml` 的 `models` 列表里对应几行删掉。
